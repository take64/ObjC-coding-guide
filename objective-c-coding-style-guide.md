...
* 条件文には必ず波括弧(中括弧"{}")を利用する。これは処理が一行で済む場合など、波括弧が必ずしも必要でない場合も利用する。
> 波括弧がないために次の一行が意図せず実行されてしまうという。コーディングミスを防ぐため。

* また波括弧のみで一行を使い、条件文と同じ行に波括弧の始まりを書いてはいけない。
...
```
  ...
  > これは「条件式の次の行に処理を書くか、一行空けて処理を書くか」という問題を解決する一つの方法です。

* 条件式はそれだけで一行を使うようにし、ANDやORを利用する場合はその都度改行し、可読性の向上に努める。
...
```
> 条件式内で比較に利用する変数名や関数名が長い場合に、エディターによって自動改行されて表示される場合があります。それは稀に見間違いによるヒューマンエラーを引き起こします。
